...
namespace BlackJackGame.Client.Models
{
    public class Deck
    {
        ...
        public Deck()
        {
            ...
            foreach (CardSuit suit in Enum.GetValues(typeof(CardSuit)))
            {
                foreach (CardRank rank in Enum.GetValues(typeof(CardRank)))
                {
                    ...
                    string rankText = GetRankText(rank);     // "A", "2".."10", "J", "Q", "K"
                    string rankWord = GetRankWord(rank);     // "ace", "2".."10", "jack", "queen", "king"
                    int value = GetCardValue(rank);
                    string suitLower = suit.ToString().ToLower();

                    // Embedded resource pack URI matching csproj Resource includes under Controls/Images/Cards
                    string imagePath = "pack://application:,,,/BlackJackGame.Client;component/Controls/Images/Cards/" +
                                       rankWord + "_of_" + suitLower + ".png";
        ...
        private string GetRankWord(CardRank rank)
        {
            switch (rank)
            {
                case CardRank.A: return "ace";
                case CardRank.J: return "jack";
                case CardRank.Q: return "queen";
                case CardRank.K: return "king";
